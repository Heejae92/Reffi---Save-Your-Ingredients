...
enum GoogleAnalyticsSink {
    ...
    /// GA4가 자동 수집하거나 예약한 이벤트 이름. 직접 로그하면 SDK가 거부하거나 자동 이벤트와 뒤섞인다.
    /// 출처: Firebase `AnalyticsEvent*` 상수와 GA4 예약 이벤트 목록.
    ...
    nonisolated static func event(for row: Analytics.Row) -> (name: String, parameters: [String: Any])? {
        ...
        case "screen_view":
            ...
            return (AnalyticsEventScreenView, [AnalyticsParameterScreenName: String(screen.prefix(100))])
        default:
