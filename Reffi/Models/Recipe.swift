...
struct Recipe: Identifiable, Codable, Equatable {
    ...
    struct Item: Codable, Equatable {
        ...
        var en: String
        var ko: String?
        /// 인분 기준 안내용 분량. 재고 차감에는 사용하지 않으며 기존 저장 데이터에서는 nil.
        var amount: LocalizedName? = nil

        var displayAmount: String? {
            guard let amount else { return nil }
            return Recipe.isKorean ? (amount.ko ?? amount.en) : amount.en
        }
    ...
    /// (`intro`·`origin`과 같은 규칙). 재료별 안내 분량이 있으면 이 인분을 기준으로 한다.
    /// 재고의 실제 사용량이나 자동 차감 비율을 뜻하지 않는다.
