...
/// 첫 등록은 이미 냉장고에 있는 재료부터 적도록 직접 입력을 1차 버튼으로 둔다.
/// 이후 등록에서는 영수증 스캔이 주역이고, 직접 입력은 보조 링크로 남는다.
///
...
struct AddIngredientSheet: View {
    ...
    var body: some View {
        ReceiptScanView(isFirstIngredient: store.isPristine).analyticsScreen(.add)
    }
}
