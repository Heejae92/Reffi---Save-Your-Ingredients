...
- `-analyticsOff` 행동 계측(`Analytics`) 킬스위치 — 큐·업로드 전부 끈다(유닛 테스트 호스트는 인자 없이도 꺼진다). **DEBUG 빌드는 기본으로 아무것도 올리지 않는다** — GA에 보내 보려면 `-analyticsDebug`를 주고, Firebase DebugView로 보려면 `-FIRDebugEnabled`를 함께 준다. 둘 다 `GoogleService-Info.plist`가 번들에 있어야 동작한다(`docs/ANALYTICS.md`)
