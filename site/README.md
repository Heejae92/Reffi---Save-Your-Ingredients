...
- 색: `index.html`의 `:root` 토큰(`--canvas`·`--paper`·`--receipt`·`--sub`·`--ink`·`--ink2`·`--muted`·`--blue`·`--fresh`·`--soon`·`--urgent`)은 design_system.md §2.3 뉴트럴 램프와 §2.7 라이트/다크 표의 OKLCH 값을 sRGB로 옮긴 것이고, `prefers-color-scheme: dark`에서 §2.8 다크 값으로 바뀝니다(캔버스 #F8F5EC ↔ #1C1914, 잉크 #25211B ↔ #EBE7E0). 옛 이름 `--c-cream`·`--c-cocoa` 등은 이 토큰의 별칭입니다. 잉크 틴트는 `rgba(var(--ink-rgb), α)`, 그림자는 `rgba(var(--shadow-rgb), α)`(다크에서 순검정)로 씁니다. 재료 PNG와 화면 스프라이트는 스킴 불변(라이트 앱 캡처)이고 폰 목업 테두리 `--device`도 고정입니다. 토큰을 바꿀 때는 디자인 시스템 표를 정본으로 hex를 다시 계산합니다.
...
- 문구: 2026-09-10 오너 지시로 **실제 런칭 기준**으로 씁니다. 베타·TestFlight 언급은 쓰지 않습니다. 주 CTA "Download on the App Store" 두 개(`data-store-link`)의 `href`는 App Store URL이 정해지기 전까지 자리표시자 `#`이며, URL이 나오면 두 곳을 함께 교체합니다. 보조 링크 "See how Reffi works"는 오너 지시로 제거했습니다(2026-09-10).
